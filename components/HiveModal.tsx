...
import { getHiveTransmissions, deleteHiveTransmission, saveNote } from '../services/dbService';
import { HiveTransmission } from '../types';
...
const HiveModal: React.FC<HiveModalProps> = ({ username, onClose, onSvgClick }) => {
  ...
  const transmissions = useLiveQuery(() => getHiveTransmissions(username), [username]) || [];
  ...
  const acceptMutation = useMutation({
    mutationFn: async (tx: HiveTransmission) => {
      ...
      // Remove from inbox
      await deleteHiveTransmission(tx.id);
    },
    onSuccess: () => {
      // queryClient.invalidateQueries({ queryKey: ['hive', username] }); -> LiveQuery handles
      // queryClient.invalidateQueries({ queryKey: ['notes', username] }); -> LiveQuery handles (in NotesModal or elsewhere)
      setSelectedTx(null);
    }
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await deleteHiveTransmission(id);
    },
    onSuccess: () => {
