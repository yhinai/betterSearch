...
const SyllabusModal: React.FC<SyllabusModalProps> = ({ username, onClose, config, onSvgClick, onFork }) => {
  ...
  const notes = useLiveQuery(() => getNotes(username), [username]) || [];

  const savedSyllabus = useLiveQuery(() => getSyllabus(username), [username]);
  const isLoadingSyllabus = savedSyllabus === undefined;

  const saveMutation = useMutation({
    mutationFn: async ({ content, count }: { content: string, count: number }) => {
      await saveSyllabus(content, count, username);
    }
  });
